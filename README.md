...
![image](https://user-images.githubusercontent.com/29851990/147269889-4394e8f7-c939-4df7-abfc-a092829b4a80.png)

System은 프로세스를 생성하는 애 끝내는 애가 필요하다.
Kernel에서 사용자에게 프로세스를 어떻게 생성하고 끝내는지에 대한 함수를 제공해준다.

![image](https://user-images.githubusercontent.com/29851990/147269903-e1c6dfff-e956-4139-995d-7a500ab40a10.png)
...
![image](https://user-images.githubusercontent.com/29851990/147269919-d6a368d3-494c-4793-a545-90c60b9631b2.png)

fork() : 새로운 프로세스 생성
ex) pid = fork();
return value
 - 0 미만 : error
 - 0 : 새로 생성된 자식 프로세스의 코드가 수행된다. 
 - 0 초과 : 부모 프로세스의 코드가 수행된다. 

cf) 새롭게 만들어진 자식 프로세스는 부모 프로세스와 똑같이 생성되므로 부모 프로세스가 수행하던 코드 그대로 수행을 진행한다.
그러므로 return된 pid를 보고, 위의 if에서 각자 갈길을 가게된다.

exec() : 프로세스는 프로세스가 생성해야한다. 근데 완전히 다른 일을 수행하는 프로세스를 만들고 싶을때는 exec()를 사용하여 이전 코드를 싹 없애면된다.

cf) exec()를 수행하게 되면 앞서 보았던 heap, stack, data, text들이 cancel된다. 그리고 새로 Load한 코드를 수행한다.
따라서 exec() 밑에 쓰는 것은 절대 수행되지 않는다. (다 날려버리기 때문에)

하지만 OS단에서는 부모 프로세스와 자식 프로세스의 관계성은 남는다.
wait() : 프로세스가 끝나면 Terminate 상태로 가도 잠시 대기한다.
...
![image](https://user-images.githubusercontent.com/29851990/147270001-72c1a1dd-f6ea-4920-8305-0268784f3699.png)
...
![image](https://user-images.githubusercontent.com/29851990/147270022-b42b3d59-bf9b-483b-8ecd-ea06c91191a4.png)

프로세스들끼리 데이터를 공유 해야될 수도 있음.
하지만 프로세스들끼리는 메모리가 구분이 되어있음.
공유할 수 있는 방법 IPC이다.
IPC
 - Shared memory
   : 메모리공간을 공유해주는 방법
...
![image](https://user-images.githubusercontent.com/29851990/147270044-dc5d7f6d-9737-4294-a956-7e3a5f98ecb4.png)
...
내 생각) 그냥 데이터 업데이트하는 변수에 대한 Boolean값 하나 있으면 되지않을까

link 
 - unidirectional : 한쪽으로 링크연결
 - bi-directional : 양쪽으로 링크연결

![image](https://user-images.githubusercontent.com/29851990/147270115-d4eea784-8ded-4062-b0e5-ecdeb92a61d4.png)

특정한 애한테 걔만 보라고 명시적으로 찍어주는것
- 링크가 자동적으로 생성이됨. (양방향이기 때문에)

![image](https://user-images.githubusercontent.com/29851990/147270127-de41f256-2ac8-405f-81bc-9175b818ddd6.png)

위의 Commuication box안에 넣어놓는다. (mailboxes라고 많이 부름)
알람을 받은 애는 누가 쓴지는 모른다.

![image](https://user-images.githubusercontent.com/29851990/147270147-a47f4125-58ac-4c7a-9c27-34e7a3f0ec11.png)

Blocking : 내가 데이터를 보내거나 받을 때, 상대방이 받았다고 말하기 전까지 또는 상대방에게 데이터를 받을 때까지 뒤의 코드를 수행하지 않는다.

Non-Blocking : 데이터를 상대방이 받던 말던, 상대방한테 데이터가 오던 말던 상관없이 진행한다.

![image](https://user-images.githubusercontent.com/29851990/147270163-574a443c-0906-4313-b7c2-5ff85781b49a.png)
